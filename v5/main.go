...
func requestGPT() (messageID string, model string, respCh chan string) {
	...
	go func() {
		// it ensures that we cancel the context after finishing handling the body of stream response
		// however, the duration we set in context.WithTimeout will also terminate the execution of handling the body of stream response
		defer cancel()
